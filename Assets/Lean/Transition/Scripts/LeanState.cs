...
{
	/// <summary>
	///     This is the base class for all transition states. When you register a transition (e.g.
	///     LeanTransformLocalPosition), it will return an instance of this class, allowing it to be updated by the transition
	///     manager.
	/// </summary>
	public abstract class LeanState
    {
		...
		/// <summary>
		///     If this is enabled then the current transition will no longer update, but will otherwise act as normal and not
		///     be removed. This is so any transition chain can still work as expected.
		/// </summary>
        ...
        [NonSerialized] public List<LeanState> Next = new List<LeanState>();

        /// <summary>If this transition is chained to another, then this tells you which must finish before this can begin.</summary>
        [NonSerialized] public List<LeanState> Prev = new List<LeanState>();
        ...
        public void SkipAll()
        {
            ...
            foreach (LeanState state in Next)
                state.SkipAll();
        }
