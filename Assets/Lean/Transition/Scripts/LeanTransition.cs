...
namespace Lean.Transition
{
    ...
    public class LeanTransition : MonoBehaviour
    {
        ...
        private static readonly List<LeanState> unscaledUpdateStates = new List<LeanState>();

        private static readonly List<LeanState> unscaledLateUpdateStates = new List<LeanState>();

        private static readonly List<LeanState> unscaledFixedUpdateStates = new List<LeanState>();

        private static readonly List<LeanState> updateStates = new List<LeanState>();

        private static readonly List<LeanState> lateUpdateStates = new List<LeanState>();

        private static readonly List<LeanState> fixedUpdateStates = new List<LeanState>();

        private static readonly List<LeanMethod> tempBaseMethods = new List<LeanMethod>();

        private static readonly List<LeanMethod> baseMethodStack = new List<LeanMethod>();

        private static readonly Dictionary<string, Type> aliasTypePairs = new Dictionary<string, Type>();
        ...
        [SerializeField] [FormerlySerializedAs("Timing")] private LeanTiming defaultTiming = LeanTiming.UnscaledUpdate;

        /// <summary>This allows you to set where in the game loop animations are updated when timing = LeanTime.Default.</summary>
        public LeanTiming DefaultTiming
        {
        ...
        public static LeanState PreviousState { get; private set; }

	    /// <summary>
	    ///     If you want the next registered transition state to automatically begin after an existing transition state,
	    ...
	    /// <summary>
	    ///     This allows you to change where in the game loop all future transitions in the current animation will be
	    ///     updated.
	    /// </summary>
        ...
        public static float CurrentSpeed { set; get; } = 1.0f;

	    /// <summary>
	    ///     This allows you to change the alias name to UnityEngine.Object association of all future transitions in the
	    ///     current animation.
	    /// </summary>
	    public static Dictionary<string, Object> CurrentAliases { get; } = new Dictionary<string, Object>();
        ...
        public static event Action<LeanState> OnRegistered;

        public static event Action<LeanState> OnFinished;
	    ...
	    /// <summary>
	    ///     This method will return the specified timing, unless it's set to <b>Default</b>, then it will return
	    ///     <b>UnscaledTime</b>.
	    /// </summary>
        ...
        {
            return (LeanTiming) Math.Abs((int) current);
        }
        ...
        public static void RequireSubmitted()
        {
            ...
            {
                Debug.LogError("You forgot to submit the last transition! " +
        ...
        public static void InsertTransitions(Transform root, float speed = 1.0f, LeanState parentHead = null)
        {
            if (root != null)
            {
                ...
                LeanState childParentHead = PreviousState;
        ...
        public static Dictionary<string, Type> FindAllAliasTypePairs(Transform root)
        {
        ...
        private static void AddAliasTypePairs(Transform root)
        {
            if (root != null)
            {
                ...
                for (int i = 0; i < tempBaseMethods.Count; i++)
                {
                    LeanMethodWithStateAndTarget baseMethod = tempBaseMethods[i] as LeanMethodWithStateAndTarget;

                    if (baseMethod != null)
                    {
                        Type targetType = baseMethod.GetTargetType();
                        string alias = baseMethod.Alias;

                        if (string.IsNullOrEmpty(alias) == false)
                        {
                            Type existingType = default;
                            ...
                            if (aliasTypePairs.TryGetValue(alias, out existingType))
                            {
                                ...
                                if (existingType != targetType)
                                {
                                    ...
                                    Debug.LogError(
                                        ...
                                        ") + (" +
                                        targetType +
                                        "). You must give them different aliases.", root);
                                }
                            }
                            // Add new?
                            else
        ...
        public static LeanState Register(LeanState state, float duration)
        {
            ...
            LeanTiming finalUpdate = GetTiming(currentTiming);
	    ...
	    /// <summary>
	    ///     This method will mark all transitions as Skip = true if they match the transition type and target object of
	    ///     the specified transition.
	    /// </summary>
	    private void RemoveConflictsBefore(List<LeanState> states, LeanState currentState, int currentIndex)
        {
            LeanState.ConflictType currentConflict = currentState.Conflict;

            if (currentConflict != LeanState.ConflictType.None)
            {
                Type currentType = currentState.GetType();
                Object currentTarget = currentState.GetTarget();

                for (int i = 0; i < currentIndex; i++)
                {
                    LeanState transition = states[i];
        ...
        private void UpdateAll(List<LeanState> states, float delta)
        {
            ...
            {
                LeanState state = states[i];

                // If we have a negative duration, skip ahead of time?
                if (state.Prev.Count > 0 && state.Duration < 0.0f)
                {
                    float skip = -state.Duration;

                    for (int j = state.Prev.Count - 1; j >= 0; j--)
                    {
                        LeanState prev = state.Prev[j];
        ...
        private static void FinishState(LeanState state)
        {
            ...
            for (int j = state.Next.Count - 1; j >= 0; j--)
...
#if UNITY_EDITOR
	    /// <summary>
	    ///     If a transition is being animated in the editor, then the target object may not update, so this method will
	    ///     automatically dirty it so that it will.
	    /// </summary>
	    private static void DirtyTarget(LeanState transition)
        {
            if (Application.isPlaying == false)
            {
                FieldInfo targetField = transition.GetType().GetField("Target");

                if (targetField != null)
                {
                    Object target = targetField.GetValue(transition) as Object;
...
namespace Lean.Transition.Editor
{
    ...
    public class LeanTransition_Editor : LeanEditor
    {
        ...
        {
            GameObject gameObject = new GameObject(typeof(LeanTransition).Name);
