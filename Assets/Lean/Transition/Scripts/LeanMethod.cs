...
namespace Lean.Transition
{
    ...
    public abstract class LeanMethod : MonoBehaviour
    {
	    ...
	    /// <summary>
	    ///     This will take the input linear 0..1 value, and return a transformed version based on the specified easing
	    ///     function.
	    /// </summary>
	    public static float Smooth(LeanEase ease, float x)
        {
            switch (ease)
            {
                ...
                {
                    float angle = x * Mathf.PI * 4.0f;
                    float weightA = 1.0f - Mathf.Pow(x, 0.125f);
                    float weightB = 1.0f - Mathf.Pow(1.0f - x, 8.0f);
    ...
    {
        [FormerlySerializedAs("Data.TargetAlias")] public string Alias;
            ...
            where T : Object
        {
            if (string.IsNullOrEmpty(Alias) == false)
            {
                ...
                if (LeanTransition.CurrentAliases.TryGetValue(Alias, out target))
                {
                    if (target is T)
                        return (T) target;
...
namespace Lean.Transition.Editor
{
    ...
    {
        private static readonly List<LeanMethod> tempMethods = new List<LeanMethod>();

        private bool IsLast
        {
            get
            {
                LeanMethod tgt = (LeanMethod) target;
        ...
        private void DrawTargetAlias(SerializedProperty sTarget, SerializedProperty sAlias, FieldInfo data)
        {
            TARGET tgt;
            TARGET[] tgts;
            ...
            if (data != null)
            {
                LeanState state = data.GetValue(tgt) as LeanState;
            ...
            bool setTarget = sTarget.objectReferenceValue != null;
            bool setAlias = string.IsNullOrEmpty(sAlias.stringValue) == false;
            int setCount = (setTarget ? 1 : 0) + (setAlias ? 1 : 0);
            ...
            if (setAlias)
            {
                LeanMethodWithStateAndTarget methodST = (LeanMethodWithStateAndTarget) tgt;
                Type expectedType = methodST.GetTargetType();
                string expectedAlias = sAlias.stringValue;

                foreach (LeanMethodWithStateAndTarget method in tgt.GetComponents<LeanMethodWithStateAndTarget>())
                {
                    Type methodTargetType = method.GetTargetType();

                    if (methodTargetType != expectedType)
                        if (method.Alias == expectedAlias)
                        {
                            if (methodTargetType.IsSubclassOf(typeof(Component)) &&
                                expectedType.IsSubclassOf(typeof(Component)))
                                continue;
        ...
        private void DrawAutoFill(TARGET tgt, FieldInfo data)
        {
            LeanState state = data.GetValue(tgt) as LeanState;
        ...
        protected override void OnInspector()
        {
            ...
            {
                SerializedProperty sTarget = serializedObject.FindProperty("Data.Target");
                SerializedProperty sAlias = serializedObject.FindProperty("Alias");
                FieldInfo data = tgt.GetType().GetField("Data");
                ...
                while (true)
                {
                    if (dataProperty.name != "Duration" &&
                        dataProperty.name != "Target" &&
            ...
            if (tgt is LeanJoin || tgt is LeanJoinDelay || tgt is LeanJoinInsert)
                if (IsLast)
