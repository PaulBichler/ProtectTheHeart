...
namespace Lean.Transition.Method
{
    ...
    [HelpURL(LeanTransition.HelpUrlPrefix + "LeanTransformPosition_X")]
    [AddComponentMenu(
        LeanTransition.MethodsMenuPrefix +
        "Transform/Transform.position.x" +
        LeanTransition.MethodsMenuSuffix +
        "(LeanTransformPosition_X)")]
...
namespace Lean.Transition
{
    ...
    {
        public static TARGET positionTransition_x(this TARGET target, float value, float duration,
            LeanEase ease = LeanEase.Smooth)
        {
            LeanTransformPosition_X.Register(target, value, duration, ease);
            return target;
        }
    }
}