...
namespace Lean.Transition.Method
{
    ...
    [AddComponentMenu(
        LeanTransition.MethodsMenuPrefix + "JoinDelay" + LeanTransition.MethodsMenuSuffix + "(LeanJoinDelay)")]
...
namespace Lean.Transition
{
    ...
    {
	    /// <summary>
	    ///     This allows you to connect the previous and next transitions, and insert a delay. This means the next
	    ///     transition will only begin when the previous one finishes.
	    /// </summary>
	    public static T JoinDelayTransition<T>(this T target, float delay)
            where T : Component
        {
            LeanTransition.CurrentQueue = LeanTransition.PreviousState;
            LeanDelay.Register(delay);
            LeanTransition.CurrentQueue = LeanTransition.PreviousState;
            return target;
        }
