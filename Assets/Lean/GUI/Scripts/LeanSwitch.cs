...
namespace Lean.Gui
{
    ...
    public class LeanSwitch : MonoBehaviour
    {
        ...
        [SerializeField] private LeanPlayer changedStateTransitions;
        [SerializeField] private IntUnityEvent onChangedState;

        [NonSerialized] private LinkedListNode<LeanSwitch> link;
	    ...
	    public List<LeanPlayer> States
        {
            get
            {
                if (states == null) states = new List<LeanPlayer>();
                return states;
            }
        }
	    ...
	    ///     For example, the <b>LeanPlaySound (Play Sound Transition)</b> component can be used to play a switch sound.
	    /// </summary>
	    public LeanPlayer ChangedStateTransitions
        {
            get
            {
                if (changedStateTransitions == null) changedStateTransitions = new LeanPlayer();
                return changedStateTransitions;
            }
        }

        /// <summary>This allows you to perform an action when this switch changes to a different state.</summary>
        public IntUnityEvent OnChangedState
        {
            get
            {
                if (onChangedState == null) onChangedState = new IntUnityEvent();
	    ...
	    /// <summary>
	    ///     This allows you to switch to a different state, where 0 is the first state. The amount of states is defined by
	    ///     the size of the <b>Transitions</b> list.
	    /// </summary>
	    public void Switch(int newState)
        {
            if (states != null && newState >= 0 && newState < states.Count)
                if (state != newState)
                {
                    ...
                    LeanPlayer stateTransitions = states[state];
        ...
        {
            LinkedListNode<LeanSwitch> node = Instances.First;

            for (int i = Instances.Count - 1; i >= 0; i--)
            {
                LeanSwitch instance = node.Value;
...
namespace Lean.Gui.Editor
{
    ...
    public class LeanSwitch_Editor : LeanEditor
    {
        ...
        protected override void OnInspector()
        {
            ...
            DrawState(tgt, tgts);
            Draw("states",
                "This stores a list of all switch transition states. This controls how many states can be switched between. For example, if you want to be able to switch between 4 states, then make sure the size of this list = 4.");

            Separator();

            Draw("changedStateTransitions",
                "This allows you to perform a transition when this switch changes to a different state. You can create a new transition GameObject by right clicking the transition name, and selecting Create. For example, the LeanPlaySound (Play Sound Transition) component can be used to play a switch sound.");
