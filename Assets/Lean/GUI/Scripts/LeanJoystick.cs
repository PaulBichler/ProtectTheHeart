...
namespace Lean.Gui
{
    ...
    public class LeanJoystick : LeanSelectable, IPointerDownHandler, IPointerUpHandler
    {
        ...
        [SerializeField] private Vector2 size = new Vector2(25.0f, 25.0f);
        [SerializeField] private float radius = 25.0f;
        [SerializeField] private RectTransform handle;
        [FSA("Dampening")] [SerializeField] private float damping = 5.0f;
        ...
        protected virtual void Update()
        {
            ...
            if (pointer != null)
            {
                if (IsInteractable())
                {
                    if (RectTransformUtility.ScreenPointToLocalPointInRectangle(CachedRectTransform, pointer.position,
                        pointer.pressEventCamera, out value))
                    {
                        value -= offset;
            ...
            if (relativeToOrigin && relativeRect != null)
        ...
        public override void OnPointerDown(PointerEventData eventData)
        {
            if (pointer == null && IsInteractable())
            {
                ...
                Vector2 origin = pointer.position;

                if (relativeToOrigin == false)
                {
                    Vector3 worldPoint = transform.TransformPoint(CachedRectTransform.rect.center);
...
namespace Lean.Gui.Editor
{
    ...
    public class LeanJoystick_Editor : LeanSelectable_Editor
    {
        ...
        protected override void DrawSelectableEvents(bool showUnusedEvents)
        {
            ...
            if (showUnusedEvents || Any(tgts, t => t.OnSet.GetPersistentEventCount() > 0))
                Draw("onSet");

            if (showUnusedEvents || Any(tgts, t => t.OnUp.GetPersistentEventCount() > 0))
                Draw("onUp");
