...
namespace Lean.Gui
{
    ...
    public class LeanPulse : MonoBehaviour
    {
        ...
        [SerializeField] private int remainingPulses = -1;
        [SerializeField] private float remainingTime;
        [SerializeField] private float timeInterval = 1.0f;
        [SerializeField] private LeanTiming timing;
        [SerializeField] private LeanPlayer pulseTransitions;
        [SerializeField] private UnityEvent onPulse;

        [NonSerialized] private LinkedListNode<LeanPulse> link;
	    ...
	    ///     For example, the <b>LeanPlaySound (Play Sound Transition)</b> component can be used to play a pulse sound.
	    /// </summary>
	    public LeanPlayer PulseTransitions
        {
            get
            {
                if (pulseTransitions == null) pulseTransitions = new LeanPlayer();
                return pulseTransitions;
            }
        }
        ...
        {
            LeanTiming finalUpdate = LeanTransition.GetTiming(timing);
        ...
        {
            LinkedListNode<LeanPulse> node = Instances.First;

            for (int i = Instances.Count - 1; i >= 0; i--)
            {
                LeanPulse instance = node.Value;
