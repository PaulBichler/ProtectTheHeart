...
namespace Lean.Common
{
    ...
    public class LeanSelect : MonoBehaviour
    {
        ...
        [FSA("MaxSelectables")] [SerializeField] private int maxSelectables = 5;
...
namespace Lean.Common.Editor
{
    ...
    public class LeanSelect_Editor : LeanEditor
    {
            ...
            LeanSelectable select =
                (LeanSelectable)
                    EditorGUILayout.ObjectField(
                        new GUIContent("Select", "Drop a selectable object here to select it."), null,
        ...
        {
            if (Any(tgts, t => t.OnSelected.GetPersistentEventCount() > 0) || showUnusedEvents)
                Draw("onSelected");

            if (Any(tgts, t => t.OnDeselected.GetPersistentEventCount() > 0) || showUnusedEvents)
                Draw("onDeselected");

            if (Any(tgts, t => t.OnNothing.GetPersistentEventCount() > 0) || showUnusedEvents)
                Draw("onNothing");
        }
    }
}
