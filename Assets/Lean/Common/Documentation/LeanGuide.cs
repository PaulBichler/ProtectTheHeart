...
namespace Lean.Common
{
    ...
    public class LeanGuide : ScriptableObject
    {
        ...
        [NonSerialized] private string version;
        ...
        public string Version
        {
            get
            {
                if (version == null && Documentation != null)
                {
                    ...
                    int a = text.IndexOf("Documentation - ");
...
namespace Lean.Common.Editor
{
    ...
    public class LeanGuide_Editor : UnityEditor.Editor
    {
        ...
        {
            LeanGuide tgt = (LeanGuide) target;
            ...
            if (GUILayout.Button(new GUIContent("Local Documentation", "Open In Web Browser")))
                Process.Start(Application.dataPath +
                              AssetDatabase.GetAssetPath(tgt.Documentation).Remove(0, "Assets".Length));

            if (GUILayout.Button(new GUIContent("Online Documentation", "Open In Web Browser")))
            ...
            if (GUILayout.Button(new GUIContent("Forum Thread", "Unity Forums")))
                Application.OpenURL("https://CarlosWilkes.com/Forum/" + tgt.ShortName);

            if (GUILayout.Button(new GUIContent("E-Mail Me", "carlos.wilkes@gmail.com")))
                Application.OpenURL("mailto:carlos.wilkes@gmail.com");

            if (GUILayout.Button(new GUIContent("Private Message", "Unity Forum Profile")))
            ...
            EditorGUILayout.LabelField("If you haven't already, consider rating this asset. It really helps me out!",
                bodyStyle);

            if (GUILayout.Button(new GUIContent("Rate This Asset", tgt.LongName + " Asset Page")))
            ...
            EditorGUILayout.LabelField(
                "I've released a range of assets to speed up your project development, check them out:", bodyStyle);

            if (GUILayout.Button(new GUIContent("My Website", "CarlosWilkes.com")))
        ...
        protected override void OnHeaderGUI()
        {
            ...
            GUILayout.BeginHorizontal("In BigTitle");
            {
                ...
                GUIContent content = new GUIContent("Version\n" + tgt.Version);
                float height = Mathf.Max(
                    titleStyle.CalcHeight(content, EditorGUIUtility.currentViewWidth - iconWidth), iconWidth);

                if (tgt.Icon != null)
                    GUILayout.Label(tgt.Icon, EditorStyles.centeredGreyMiniLabel, GUILayout.Width(iconWidth),
