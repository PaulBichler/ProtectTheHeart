...
namespace GAP_ParticleSystemController
{
    ...
    public class ParticleSystemController : MonoBehaviour
    {
        ...
        private readonly List<ParticleSystemOriginalSettings> psOriginalSettingsList =
            new List<ParticleSystemOriginalSettings>();
        ...
        public void UpdateParticleSystem()
        {
            ...
            for (int i = 0; i < ParticleSystems.Count; i++)
                ...
                {
                    Debug.Log(
                        "Make sure the ActiveParticleSystems list has the same amount as the ParticleSystems list.");
                    return;
                }
            ...
            if (ParticleSystems.Count > 0)
            {
                for (int i = 0; i < ParticleSystems.Count; i++)
                {
                    ...
                    if (ps != null)
                    {
                        ...
                        ParticleSystem.LightsModule psLights = ps.lights;
                        ParticleSystem.TrailModule psTrails = ps.trails;
                        ParticleSystem.ColorOverLifetimeModule colorOverLifetime = ps.colorOverLifetime;
                        ParticleSystem.MinMaxGradient colorOverLifetimeC = colorOverLifetime.color;
                        ...
                        ParticleSystem.MinMaxCurve velocityOverLifetimeX = velocityOverLifetime.x;
                        ParticleSystem.MinMaxCurve velocityOverLifetimeY = velocityOverLifetime.y;
                        ParticleSystem.MinMaxCurve velocityOverLifetimeZ = velocityOverLifetime.z;
                        Vector3 localPos = ParticleSystems[i].transform.localPosition;
                        ...
                        {
                            List<ParticleSystemOriginalSettings> listOriginalSettings =
                                new List<ParticleSystemOriginalSettings>();
                        ...
                        //DURATION
                        if (duration != 1) main.duration *= duration;
                        ...
                        //RADIUS
                        if (shape.enabled) shape.radius *= size;
                        ...
                        if (changeColor)
                        {
                            ...
                            if (colorOverLifetime.enabled)
                            {
                                if (colorOverLifetime.color.mode == ParticleSystemGradientMode.Gradient)
                                    colorOverLifetimeC.gradient = ChangeGradientColor(colorOverLifetimeC.gradient,
                                        newMaxColor, newMinColor);
                                if (colorOverLifetime.color.mode == ParticleSystemGradientMode.TwoGradients)
                                {
                                    colorOverLifetimeC.gradientMax = ChangeGradientColor(
                                        colorOverLifetimeC.gradientMax, newMaxColor, newMinColor);
                                    colorOverLifetimeC.gradientMin = ChangeGradientColor(
                                        colorOverLifetimeC.gradientMin, newMinColor, newMaxColor);
                                }
                    ...
                    {
                        //TRAIL RENDERER
                        TrailRenderer trail = ParticleSystems[i].GetComponent<TrailRenderer>();
                        if (trail != null)
                        {
                            if (!SaveParticleSystemScript.CheckExistingFile(gameObject))
                            {
                                ParticleSystemOriginalSettings psOriginalSettings =
                                    new ParticleSystemOriginalSettings {
        ...
        {
            if (ParticleSystems.Count == 0) FillLists();
            ...
            if (listOriginalSettings != null)
            {
                for (int i = 0; i < ParticleSystems.Count; i++)
                {
                    ...
                    if (ps != null)
                    {
                        ...
                        ParticleSystems[i].transform.localPosition =
                            listOriginalSettings[i]._localPosition.GetVector3();
                        main.duration = listOriginalSettings[i]._duration;
                        shape.radius = listOriginalSettings[i]._shapeRadius;
                        ps.gameObject.SetActive(listOriginalSettings[i]._active);
        ...
        public Color ChangeHUE(Color oldColor, Color newColor)
        {
            ...
            Color updatedColor = Color.HSVToRGB(newHue, oldSaturation, oldValue);
        ...
        public void FillLists()
        {
            if (ParticleSystems.Count == 0)
            {
                ...
                for (int i = 0; i < ParticleSystems.Count; i++) ActiveParticleSystems.Add(true);
            }
            else
            {
                Debug.Log(
                    "Lists already have GameObjects. For automatic filling consider emptying the lists and try again.");
            }
        }
        ...
        private void AddChildRecurvsively(Transform transf)
        {
            foreach (Transform t in transf)
            {
                ...
                TrailRenderer trailChild = child.GetComponent<TrailRenderer>();
