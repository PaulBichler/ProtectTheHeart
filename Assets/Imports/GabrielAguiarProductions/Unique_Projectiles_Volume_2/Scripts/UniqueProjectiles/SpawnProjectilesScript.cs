...
public class SpawnProjectilesScript : MonoBehaviour
{
    ...
    private readonly List<Camera> camerasList = new List<Camera>();
    ...
    private void Start()
    {
        if (cameras.transform.childCount > 0)
        {
            ...
            if (camerasList.Count == 0) Debug.Log("Please assign one or more Cameras in inspector");
        }
        else
        {
    ...
    public void SpawnVFX()
    {
        ...
        CameraShakeSimpleScript cameraShakeScript = cameras.GetComponent<CameraShakeSimpleScript>();
        ...
        if (camerasList.Count > 0)
        {
            if (!camerasList[0].orthographic)
            {
                if (camerasList[0].fieldOfView < 101)
                    for (int i = 0; i < camerasList.Count; i++) camerasList[i].fieldOfView += 5;
            }
            else
            {
                if (camerasList[0].orthographicSize < 10)
                    for (int i = 0; i < camerasList.Count; i++) camerasList[i].orthographicSize += 0.5f;
            }
        }
    }
